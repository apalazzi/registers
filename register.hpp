...
template <typename T, uint_t pos0, uint_t len,
          uint_t step= len,
          uint_t posN= (8 * sizeof(T) - pos0) / step,
          RegisterType RT= RegisterType::read_write,
          typename IDX= uint_t>
class BitArray {
   ...
   public:
    ...
    T get(const uint_t idx, const std::nothrow_t nothrow
    ...
    class reference {
       ...
       public:
        ...
        operator T() const { return bf.get(idx); }
    ...
    T operator[](
        const IDX idx) const {  // FIXME: constexpr?
    ...
    // TODO: add some range check if IDX is int type to
    // avoid accessing bits that should not be accessed
   private:
    volatile T *const raw;
    ...
    constexpr void check() const noexcept {
        ...
        static_assert(pos0 + len <= sizeof(T) * 8,
